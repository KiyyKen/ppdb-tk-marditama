...
<div class="md:w-9/12 bg-white p-6 rounded-2xl shadow-xl border border-gray-100">
    <div class="flex justify-between items-center border-b pb-4 mb-6">
        <div>
            <h2 class="text-2xl font-black text-gray-800">Edit Data Admin</h2>
            <p class="text-sm text-gray-500">Perbarui username atau password untuk pengguna <strong>{{ $admin->username }}</strong>.</p>
        </div>
    ...
    <div class="max-w-lg mx-auto bg-gray-50 p-6 rounded-2xl border border-gray-200">
        ...
        <form action="{{ route('admin.update', $admin->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="username" value="{{ old('username', $admin->username) }}" class="w-full px-4 py-2.5 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" required>
            </div>

            <div class="mb-6">
                <label class="block text-sm font-bold text-gray-700 mb-1">Password Baru (Biarkan kosong jika tidak diubah)</label>
                <input type="password" name="password" class="w-full px-4 py-2.5 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="Isi hanya jika ingin mengganti password">
            </div>
